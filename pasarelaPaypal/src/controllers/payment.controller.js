...
export const captureOrder = async (req, res) => {
  ...
  try {
    ...
    if (response.data.status === "COMPLETED") {
      // Pago aprobado
      return res.redirect(`https://cartastarotbolivia.com/descripcion-cartas?status=COMPLETED`);
    } else {
      // Pago no aprobado
      return res.redirect(`https://cartastarotbolivia.com/descripcion-cartas?status=NOT_COMPLETED`);
    }
  } catch (error) {
    if (error.response) {
      ...
      if(error.response.status === 422){
        ...
        return res.redirect(`https://cartastarotbolivia.com/descripcion-cartas?status=NOT_COMPLETED`);//cambiar esta url por una cuando se tenga el front que va a manejar errores 
      }
